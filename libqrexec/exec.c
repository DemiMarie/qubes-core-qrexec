...
#define EXTEND_RAW(arg)                                     \
    ...
    if (use_shell) {
        /* Move the command to after 'sh -lc "exec \"\$@\"" sh ' */
        argv[4] = argv[0];
        argv[5] = argv[1];
        argv[6] = NULL;
        /* Now fill in the shell invocation itelf */
        argv[0] = "sh";
        argv[1] = "-lc";
        /* Do not use "-lcexec \"$@\"": -c does not take an argument
         * itself, and instead causes the first non-option argument
